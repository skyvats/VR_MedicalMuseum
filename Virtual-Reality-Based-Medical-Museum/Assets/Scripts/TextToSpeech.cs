...
public class TextToSpeech : MonoBehaviour
{
    ...
    void Awake()
    {
        updateposition = GameObject.FindObjectOfType<UpdatePosition>();
    }
...
//It seems that AudioSource.isPlaying doesn't work reliably.
    ...
    public void AddTextToQueue(string text)
    {
        ...
        if (!string.IsNullOrEmpty(text))
        {
            ...
            if (subs[1] == "urinary" )
            {
                updateposition.UpdatePositionUrinary();
            }
            else if (subs[1] == "kidneys")
            {
                updateposition.UpdatePositionUrinary();
            }
            else if (subs[1] == "bladder")
            {
                updateposition.UpdatePositionUrinary();
            }
            else if (subs[1] == "ureter")
            {
                updateposition.UpdatePositionUrinary();
            }
            else if(subs[1] == "urethra")
            {
                updateposition.UpdatePositionUrinary();
            }
        }
